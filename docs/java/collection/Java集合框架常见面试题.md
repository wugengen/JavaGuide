...
- **4. 是否支持快速随机访问：** LinkedList 不支持高效的随机元素访问，而 ArrayList 支持。快速随机访问就是通过元素的序号快速获取元素对象(对应于`get(int index) `方法)。（注：LinkedList有`get(int index) `方法，但是看其get方法的实现可知，也是遍历了整个链表得出的元素对象）
